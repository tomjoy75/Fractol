...
/*   Updated: 2024/02/05 22:51:41 by tjoyeux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int	get_fractal_point_iter(int fractal_mode, t_complex n, t_draw draw)
{
	int	iter;
...
void	render_fractal(t_img *img, t_draw draw, int fractal_mode)
{
	...
	while (y < WINDOW_HEIGHT)
	{
		...
		while (x < WINDOW_WIDTH)
		{
			...
			iter = get_fractal_point_iter(fractal_mode, n, draw);
			if (!iter)
